 ...
 * 原则：
 *   1. 响应体必须是合法 JSON；
 *   2. 任何 PHP 致命错误、未捕获异常、Parse 错误都由 Bootstrap.php 注册的
 *      全局处理器接管，绝不会直接输出 HTML 或警告文本到前端。
 */
...
if (!$installed) {
    Response::error('系统未安装，请先完成安装（执行 bin/install-cli.php）', 503, 503);
}
